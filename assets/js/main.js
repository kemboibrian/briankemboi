...
(function ($) {
    ...
    preloaderWithBannerActivation: function () {
      ...
      if (preloaderActives.length) {
        ...
        let LoadingCounter = setInterval(function () {
          if (percentage <= 100) {
            $("#loading-screen .loading-counter").text("Web Developer");
  ...
  document.getElementById('whatsappForm').addEventListener('submit', function(event) {
    ...
    // 1. Get the values from each field
    ...
    // Use \n for new lines, and encodeURIComponent for safe URL encoding
    let whatsappMessage = `Hello Brian,\n\n` +
                         `My Name: ${nameValue}\n` +
                         `My Email: ${emailValue}\n` +
                         `My Phone: ${phoneValue}\n` +
                         `Service Interested: ${serviceValue}\n` +
                         `Message: ${messageValue}`;

    // 3. Encode the message and create the final URL
    const encodedMessage = encodeURIComponent(whatsappMessage);
    const whatsappNumber = '254715699966'; // without + sign
